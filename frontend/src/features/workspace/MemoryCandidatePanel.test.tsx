...
describe("MemoryCandidatePanel", () => {
  ...
  it("passes the selected model profile to extraction", async () => {
    ...
    await waitFor(() => {
      expect(apiClient.extractMemories).toHaveBeenCalledWith(
        "sv-1",
        "profile-1",
      );
    });
  ...
  it("lists candidates and approves a pending candidate", async () => {
    ...
    expect(
      screen.getByText("The protagonist sees the hidden letter."),
    ).toBeInTheDocument();

    fireEvent.click(screen.getByLabelText("批准记忆候选"));

    await waitFor(() => {
      expect(apiClient.updateCandidate).toHaveBeenCalledWith(
        "candidate-1",
        "approved",
      );
    });
  ...
  it.each(candidateActions)(
    ...
    async ({ status, buttonLabel }) => {
      ...
      renderWithQuery(
        <MemoryCandidatePanel sceneVersionId="sv-1" />,
        queryClient,
      );
