...
export function SceneCardEditor({
  ...
  function startEdit() {
    ...
    FIELD_LABELS.forEach((f) => {
      vals[f.key] = String(
        (scene as unknown as Record<string, unknown>)[f.key] ?? "",
      );
    });
  ...
  return (
    <section className="rounded-md border border-slate-200 bg-white p-3 text-xs">
      ...
      <div className="space-y-1.5">
        ...
        <div>
          <span className="font-medium text-slate-600">POV 人物：</span>
          <span className="text-slate-500">
            {scene.pov_character_id || "未设定"}
          </span>
        </div>
        <details className="rounded border border-slate-200 px-2 py-1.5">
          <summary className="cursor-pointer font-medium text-slate-600">
            ...
            {characters.map((character) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={
                    contextLinks.data?.character_ids.includes(character.id) ??
                    false
                  }
                  onChange={() =>
                    toggleContextLink("character_ids", character.id)
                  }
                />
                {character.name}
              </label>
            ...
            {worldEntries.map((entry) => (
              <label key={entry.id} className="mt-1 flex items-center gap-1.5">
                <input
                  type="checkbox"
                  checked={
                    contextLinks.data?.world_entry_ids.includes(entry.id) ??
                    false
                  }
                  onChange={() =>
                    toggleContextLink("world_entry_ids", entry.id)
                  }
                />
                {entry.name}
              </label>
