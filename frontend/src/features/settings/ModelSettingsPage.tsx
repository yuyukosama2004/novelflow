  ...
  return (
    <main className="min-h-screen bg-slate-100">
      ...
      <div className="mx-auto max-w-3xl space-y-4 px-5 py-4">
        {profiles.data?.map((p) => (
          <div key={p.id} className={`rounded-md border bg-white p-4 ${p.is_default ? 'border-emerald-300' : 'border-slate-200'}`}>
            {editingId === p.id ? (
              <ProfileForm form={form} setForm={setForm}
                onSave={() => save.mutate({ ...form, api_key: form.api_key || undefined })}
                onCancel={() => setEditingId(null)} isSaving={save.isPending} />
            ) : (
              <div className="flex items-start justify-between">
                <div className="text-sm">
                  <div className="flex items-center gap-2">
                    <span className="font-semibold text-slate-900">{p.name || '未命名'}</span>
                    <span className="rounded bg-slate-100 px-1.5 py-0.5 text-xs text-slate-500">{PROVIDER_LABELS[p.provider] ?? p.provider}</span>
                    {p.is_default ? <span className="rounded bg-emerald-100 px-1.5 py-0.5 text-xs text-emerald-700">默认</span> : null}
                    {!p.enabled ? <span className="rounded bg-rose-100 px-1.5 py-0.5 text-xs text-rose-700">已禁用</span> : null}
                  </div>
                  <div className="mt-1 text-slate-500">
                    模型：{p.model_name || '未设定'} · Key：{p.api_key_configured ? '已配置' : '未配置'}
                    {p.base_url ? ` · ${p.base_url}` : ''}
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <button onClick={() => testProfile.mutate(p.id)}
                    className="rounded p-1.5 text-slate-400 hover:bg-slate-100" title="测试连接">
                    <RefreshCw size={14} className={testProfile.isPending ? 'animate-spin' : ''} />
                  </button>
        ...
        {adding ? (
          <div className="rounded-md border border-indigo-200 bg-white p-4">
            <ProfileForm form={form} setForm={setForm}
              onSave={() => save.mutate(form)} onCancel={() => setAdding(false)}
              isSaving={save.isPending} />
          </div>
        ) : null}

        {testResult ? (
          <div className="rounded-md border border-slate-200 bg-white p-3 text-sm text-slate-700">{testResult}</div>
        ) : null}
...
function ProfileForm({
  form, setForm, onSave, onCancel, isSaving,
}: {
    ...
    <div className="space-y-3 text-sm">
      <label className="block">
        ...
        <input value={String(form.name ?? '')} onChange={e => setForm({ ...form, name: e.target.value })}
          placeholder="如：DeepSeek-正式写作" className="mt-1 w-full rounded border px-3 py-1.5 outline-none focus:border-emerald-600" />
      </label>
      <div className="grid grid-cols-2 gap-3">
        <label className="block">
          <span className="font-medium text-slate-700">Provider</span>
          <select value={String(form.provider ?? 'deepseek')} onChange={e => setForm({ ...form, provider: e.target.value })}
        ...
        <label className="block">
          ...
          <input value={String(form.model_name ?? '')} onChange={e => setForm({ ...form, model_name: e.target.value })}
            placeholder="如：deepseek-chat" className="mt-1 w-full rounded border px-3 py-1.5 outline-none focus:border-emerald-600" />
        </label>
      </div>
      <label className="block">
        <span className="font-medium text-slate-700">Base URL</span>
        <input value={String(form.base_url ?? '')} onChange={e => setForm({ ...form, base_url: e.target.value })}
          placeholder="留空使用默认" className="mt-1 w-full rounded border px-3 py-1.5 outline-none focus:border-emerald-600" />
      </label>
      <label className="block">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={onCancel} className="rounded border px-3 py-1.5 hover:bg-slate-100">
          <X size={14} /> 取消
        </button>
      </div>
    </div>
