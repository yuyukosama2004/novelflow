...
export function CoreConceptPanel({
  ...
  if (!project) {
    return (
      <div className="py-8 text-center text-sm text-slate-400">项目未加载</div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      {editing ? (
        ...
        <div className="grid gap-2 text-sm">
          <div>
            <span className="font-medium text-slate-600">书名：</span>
            {project.title}
          </div>
          <div>
            <span className="font-medium text-slate-600">类型：</span>
            {project.genre || "未设定"}
          </div>
          <div>
            <span className="font-medium text-slate-600">基调：</span>
            {project.tone || "未设定"}
          </div>
          <div>
            <span className="font-medium text-slate-600">视角：</span>
            {project.pov_type || "未设定"}
          </div>
          <div>
            <span className="font-medium text-slate-600">语言：</span>
