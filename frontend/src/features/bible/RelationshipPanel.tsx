      ...
      {adding ? (
        <div className="rounded-md border border-indigo-200 bg-indigo-50 p-3 space-y-2">
          ...
          <div className="flex gap-1">
            <button onClick={handleCreate} disabled={!charA || !charB || isCreating}
      ...
      {relationships.length === 0 ? (
        ...
        <div className="space-y-2">
          {relationships.map((rel) => (
            <div key={rel.id} className="flex items-start justify-between rounded-md border border-slate-200 bg-white p-3">
              <div className="min-w-0 text-sm">
                <span className="font-medium text-slate-900">{charName(rel.character_a_id)}</span>
                ...
                <span className="font-medium text-slate-900">{charName(rel.character_b_id)}</span>
                {rel.description ? <p className="mt-1 text-xs text-slate-500">{rel.description}</p> : null}
              </div>
