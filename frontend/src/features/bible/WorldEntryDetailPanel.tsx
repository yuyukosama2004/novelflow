    ...
    return (
      <div className="space-y-3">
        ...
        <label className="block text-sm">
          ...
          <input value={form.entry_type ?? ''} onChange={(e) => setForm({ ...form, entry_type: e.target.value })}
            className="mt-1 w-full rounded border border-slate-300 px-3 py-1.5 text-sm outline-none focus:border-emerald-600" />
        </label>
        <label className="block text-sm">
        ...
        <label className="block text-sm">
          ...
          <textarea value={form.content ?? ''} onChange={(e) => setForm({ ...form, content: e.target.value })}
            rows={4} className="mt-1 w-full rounded border border-slate-300 px-3 py-1.5 text-sm outline-none focus:border-emerald-600" />
        </label>
      </div>
    );
  }
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="rounded bg-slate-100 px-1.5 py-0.5 text-xs text-slate-500">{entry.entry_type}</span>
          <span className="text-xs text-slate-400">{label(CANON_STATUS_LABELS, entry.canon_status)}</span>
        </div>
      ...
      {entry.content ? <p className="text-sm text-slate-500 whitespace-pre-wrap">{entry.content}</p> : null}
    </div>
  );
}
