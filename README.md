...
写作上下文只召回当前 POV、场景显式关联、上一叙事场景关联和项目级硬规则。后续场景才获得的知识会以“禁止提前泄露”约束进入写作上下文；审查上下文保留完整受限事实。

长正文的审查和记忆提取会按段落完整分块，结果记录块号与字符范围并去重；上一场景上下文使用摘要和末尾片段，避免只读取开头。

替换已有正式稿会使旧版本产生的正史记录失效，将所有叙事后续场景标记为“需复查”，并生成可查询的影响报告；系统不会自动改写后续正文。

工作台默认展示正文、写作辅助和 AI 写作；审查、记忆、版本历史及高级上下文通过右侧中文 Tab 在两次点击内打开。

“专注写作”会隐藏左右栏和写作辅助，保留实时字数、保存状态，并允许调整字号、行距和正文宽度。

项目列表提供“快速创作”：输入一个点子即可创建完整的基础项目结构并进入普通工作台；AI 草稿会自动审查，但必须由用户明确采用，且不会自动应用正史记忆。

场景生成任务使用持久化 WorkflowRun 和数据库活动任务唯一锁；SSE 事件带递增 ID，刷新页面可恢复已有草稿，取消操作会同步写回后端。

当前仓库已验证从空 SQLite 数据库执行全部 Alembic 迁移、重复运行 seed、后端与前端测试/静态检查/构建，以及两端本地 smoke 启动。
...
- `GET /api/scenes/{scene_id}/context-links`
- `PUT /api/scenes/{scene_id}/context-links`
